...
export class HandCounter {
  ...
  load() {
    ...
    this.loading = (async () => {
      ...
      const { FilesetResolver, HandLandmarker } = await import(`${MP_DIR}/vision_bundle.mjs`);
      const fileset = await FilesetResolver.forVisionTasks(new URL(`${MP_DIR}/wasm`, import.meta.url).href);
      this.landmarker = await HandLandmarker.createFromOptions(fileset, {
        baseOptions: {
          modelAssetPath: new URL(`${MP_DIR}/hand_landmarker.task`, import.meta.url).href,
          delegate: "GPU"
        },
        runningMode: "VIDEO",
